...
def verify_candidate_frontend(base_url: str, artifact_dir: Path) -> None:
    expected_sha = os.environ.get("GITHUB_SHA", "")
    with urllib.request.urlopen(f"{base_url}/pr8326-candidate.json", timeout=20) as response:
        raw = response.read()
    payload = json.loads(raw)
    (artifact_dir / "served-candidate.json").write_bytes(raw)
    if payload.get("candidate_sha") != expected_sha:
        fail(f"served frontend SHA {payload.get('candidate_sha')!r} != workflow SHA {expected_sha!r}")
    if payload.get("candidate_version") != "2099.8.326":
        fail(f"served frontend has wrong candidate version: {payload!r}")
    pass_log(f"served frontend is the exact candidate wheel build at {expected_sha}")
...
async def main() -> None:
    ...
    try:
        if SCENARIO == "local-chat":
            ...
            verify_candidate_frontend(base_url, artifact_dir)
            password = read_bootstrap_password(home, log_path)
            if not password:
