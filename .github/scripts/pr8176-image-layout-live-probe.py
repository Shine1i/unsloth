...
async def scenario_smoke(base_url: str, password: str, browser_name: str, artifact_dir: Path) -> None:
    """Exercise the real Images route at widths on both sides of its 50rem container query."""
    init = await auth_init(base_url, password)
    async with open_chat(
        base_url,
        init_scripts=[init],
        video_dir=video_dir(artifact_dir, "images-layout"),
        video_name=f"images-layout-{browser_name}",
        transcode_mp4=False,
        browser_name=browser_name,
        viewport=(1600, 900),
    ) as sp:
        page = sp.page
        await page.goto(f"{base_url}/images", wait_until="domcontentloaded")
        tablist = page.get_by_role("tablist", name="Page mode")
        await tablist.wait_for(state="visible", timeout=30_000)

        async def set_sidebar(expanded: bool) -> None:
            label = "Open sidebar" if expanded else "Close sidebar"
            button = page.get_by_role("button", name=label).first
            if await button.is_visible():
                await button.click()
                await page.wait_for_timeout(500)

        async def capture(name: str, width: int, height: int, expanded: bool, mode: str) -> dict:
            await page.set_viewport_size({"width": width, "height": height})
            await page.wait_for_timeout(250)
            await set_sidebar(expanded)
            await page.get_by_role("tab", name=mode.capitalize()).click()
            await page.wait_for_timeout(600)
            geometry = await page.evaluate(
                """({mode}) => {
                  const surface = document.querySelector('.diffusion-surface');
                  if (!surface) throw new Error('Images surface missing');
                  const header = surface.children[0];
                  const body = surface.children[1];
                  const tabs = surface.querySelector('[role="tablist"][aria-label="Page mode"]');
                  const video = surface.querySelector('button[aria-label="Video"]');
                  const selector = header.children[0].querySelector('button');
                  if (!header || !body || !tabs || !video || !selector) throw new Error('Images layout node missing');
                  const settings = body.children[0];
                  const preview = body.children[1];
                  if (!settings || !preview) throw new Error(`${mode} pane missing`);
                  const rect = (el) => {
                    const r = el.getBoundingClientRect();
                    return {x:r.x, y:r.y, width:r.width, height:r.height, right:r.right, bottom:r.bottom, cx:r.x+r.width/2};
                  };
                  const overlap = (a, b) => Math.max(0, Math.min(a.right,b.right)-Math.max(a.x,b.x));
                  const sr=rect(surface), hr=rect(header), br=rect(body), lr=rect(header.children[0]);
                  const tr=rect(tabs), vr=rect(video), mr=rect(selector), rr=rect(settings), pr=rect(preview);
                  return {
                    mode, viewport:{width:innerWidth,height:innerHeight}, surface:sr, header:hr,
                    bodyDirection:getComputedStyle(body).flexDirection,
                    body:br, leftHeader:lr, tabs:tr, video:vr, selector:mr, settings:rr, preview:pr,
                    tabsPreviewCenterDelta:Math.abs(tr.cx-pr.cx),
                    tabsVideoOverlap:overlap(tr,vr), selectorTabsOverlap:overlap(mr,tr),
                    bodyOverflowX:body.scrollWidth-body.clientWidth,
                  };
                }""",
                {"mode": mode},
            )
            wide = geometry["surface"]["width"] >= 800
            expected_direction = "row" if wide else "column"
            if geometry["bodyDirection"] != expected_direction:
                fail(f"{name}: body direction {geometry['bodyDirection']} != {expected_direction}")
            if geometry["tabsVideoOverlap"] > 0.5 or geometry["selectorTabsOverlap"] > 0.5:
                fail(f"{name}: header controls overlap: {geometry}")
            if geometry["bodyOverflowX"] > 1:
                fail(f"{name}: body overflows horizontally by {geometry['bodyOverflowX']}px")
            if wide:
                if abs(geometry["settings"]["width"] - 408) > 1.5:
                    fail(f"{name}: settings rail is not 408px: {geometry['settings']['width']}")
                if abs(geometry["leftHeader"]["width"] - 408) > 1.5:
                    fail(f"{name}: header rail is not 408px: {geometry['leftHeader']['width']}")
            ...
            pass_log(
                f"{browser_name} {name} {mode}: surface={geometry['surface']['width']:.0f}px "
                f"direction={geometry['bodyDirection']} rail={geometry['settings']['width']:.0f}px "
                f"center-delta={geometry['tabsPreviewCenterDelta']:.1f}px"
            )
            return {"name": name, "expanded": expanded, **geometry}

        cases = [
            ("wide-expanded-1600x900", 1600, 900, True),
            ("constrained-expanded-1000x760", 1000, 760, True),
            ("wide-collapsed-1000x760", 1000, 760, False),
            ("constrained-collapsed-800x760", 800, 760, False),
        ]
        results = []
        for name, width, height, expanded in cases:
            results.append(await capture(name, width, height, expanded, "create"))
            results.append(await capture(name, width, height, expanded, "train"))
        (artifact_dir / f"geometry-{browser_name}.json").write_text(
            json.dumps(results, indent=2), encoding="utf-8"
        )
    pass_log(f"Playwright {browser_name} validated 8 live Images layouts and screenshots")
