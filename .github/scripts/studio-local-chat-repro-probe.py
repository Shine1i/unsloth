...
async def scenario_local_chat(base_url: str, api_key: str, browser_name: str, artifact_dir: Path, password: str | None) -> None:
    ...
    if not password:
        fail("could not read Studio bootstrap password for local-chat UI screenshot")
    init_scripts = [await auth_init(base_url, password)]
    async with open_chat(
        base_url,
        ...
        screenshot = artifact_dir / f"local-chat-{browser_name}.png"
        await sp.screenshot(screenshot)
        if not screenshot.is_file() or screenshot.stat().st_size == 0:
            fail(f"local-chat UI screenshot was not written: {screenshot}")
    pass_log("local small-model chat UI accepted a prompt and saved a screenshot")
