...
async def scenario_local_chat(base_url: str, api_key: str, browser_name: str, artifact_dir: Path, password: str | None) -> None:
    ...
    try:
        async with open_chat(
            ...
            await composer.wait_for(state="visible", timeout=30_000)
            await sp.screenshot(artifact_dir / f"local-chat-open-{browser_name}.png")

            # Exercise PR #6985's core path with a real loaded local model: send one
            # turn, queue a second prompt while generation is active, then verify the
            # queue UI appears and both turns drain.
            await send_prompt(sp, "Count from one to eight slowly, one number per line.")
            await page.locator('button[aria-label="Stop generating"], button:has-text("Stop")').first.wait_for(
                state="visible", timeout=60_000
            )
            await composer.fill("After that finishes, answer with exactly: queued turn complete")
            await composer.press("Enter")
            queue = page.locator('[aria-label^="Prompt queue,"]').first
            await queue.wait_for(state="visible", timeout=15_000)
            await sp.screenshot(artifact_dir / f"local-chat-queue-visible-{browser_name}.png")
            await queue.wait_for(state="hidden", timeout=300_000)
            await wait_for_stream(sp, timeout_ms=300_000)
            await page.get_by_text("queued turn complete", exact=False).first.wait_for(timeout=60_000)
            await sp.screenshot(artifact_dir / f"local-chat-queue-drained-{browser_name}.png")
        pass_log("local small-model chat UI queued and drained a prompt against a loaded model")
