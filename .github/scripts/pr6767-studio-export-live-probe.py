...
async def scenario_smoke(base_url: str, password: str, browser_name: str, artifact_dir: Path) -> None:
    """Launch Studio, authenticate, navigate to the PR-touched Export page, and screenshot it."""
    init = await auth_init(base_url, password)
    async with open_chat(
        base_url,
        init_scripts=[init],
        video_dir=video_dir(artifact_dir, "export"),
        video_name=f"export-{browser_name}",
        transcode_mp4=False,
        browser_name=browser_name,
    ) as sp:
        page = sp.page
        await page.goto(f"{base_url}/export", wait_until="domcontentloaded")
        await page.get_by_role("heading", name="Export Model").wait_for(state="visible", timeout=30_000)
        await page.get_by_text("Export Configuration").first.wait_for(state="visible", timeout=30_000)
        # The generated workflow installs Studio with --no-torch on CPU-only hosted runners; the
        # PR intentionally leaves /export reachable and self-gated, so verify that user-visible
        # state rather than trying to run a real model export here.
        await page.get_by_text("Export unavailable").first.wait_for(state="visible", timeout=30_000)
        await sp.screenshot(artifact_dir / f"studio-export-{browser_name}.png")
    pass_log(f"Playwright {browser_name} opened /export and captured the export page")
