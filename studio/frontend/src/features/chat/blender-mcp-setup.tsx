...
export function BlenderMcpSetup({ servers, disabled, onBusyChange }: {
  ...
  const refresh = useCallback(async (wait = true) => {
    ...
    try {
      ...
      if (!blender) throw new Error("Bundled Blender is not available from this Studio backend.");
      setConfig(blender);
    ...
    try {
      ...
      if (action === "test") {
        ...
        setMessage(`MCP connected (${result.tool_count} tools). ${result.blender_ready
          ? "Blender ready."
          : result.blender_error ?? "Blender is not connected. See Connect Blender below; documentation tools are still available."}`);
  ...
  return (
    <section className="space-y-4 rounded-xl border p-4" aria-label="Bundled Blender MCP">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex flex-wrap items-center gap-2">
          <h3 className="text-sm font-medium">Blender <span className="text-xs text-muted-foreground">Bundled</span></h3>
          {connection && (
      ...
      <p className="text-sm leading-relaxed text-muted-foreground">
        MCP is bundled—no server install needed. Connect Blender below to use scene tools.
      </p>
      {duplicate && <p className="text-xs leading-relaxed text-amber-600">A custom Blender server also exists. Disable it below if you prefer this bundled integration.</p>}
