...
# ============================================
# The Studio frontend and the OXC validator runtime use the same Node floor
# (Vite/OXC: Node ^20.19.0 || >=22.12.0; Studio also requires npm >= 11 for
# installs). Get-NodeDecision is pure and unit-tested in
# tests/studio/test_node_decision.ps1. It returns one of: system | bundled | skip.
...
if (-not $IsPipInstall) {
    ...
    # legacy ~\.unsloth root) so the Node nest sits beside llama.cpp. This runs even
    # when SKIP_STUDIO_FRONTEND=1 because the checked-in OXC runtime can still need
    # Node/npm for its install step.
...
if ($IsPipInstall) {
    ...
    # source before this branch (no install yet), then provision in the build/OXC
    # phase only when Node is actually needed.
...
if ($NeedNodeForSetup) {
    if ($NodeSource -eq "skip") {
        ...
        # Keep npm global installs inside the isolated Node even if nvm/npm left a
        # user-level prefix or NODE_PATH in the environment.
